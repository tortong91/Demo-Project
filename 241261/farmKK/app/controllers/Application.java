...
//    }
    ...
    public static Result update() {
        ...
        if (formupdate.hasErrors()) {
           ...
           cooperatives dataEditc =new cooperatives(session("cId"),formupdate.get().getName(),formupdate.get().getAddress(),formupdate.get().getTel(),formupdate.get().getPicture());
            cooperatives.edit(dataEditc);
            return showdata();
        }
    }
...
//    }
    ...
    public static Result inputfoods() {
        Form<Foods> myForm = myFormsfoods.bindFromRequest();
        Http.MultipartFormData body = request().body().asMultipartFormData();
        Http.MultipartFormData.FilePart picture = body.getFile("picture");
        String fileName, contentType;
        if (myForm.hasErrors()) {
            ...
            return main1(form_animalFoods.render(myForm));
                ...
                if (picture != null) {
                    ...
                    fileName = datafoods.getId() + fileName.substring(fileName.lastIndexOf("."));
                    file.renameTo(new File(picPath, fileName));
                    datafoods.setPicture(fileName);
                    myFormlistfoods.add(datafoods);
                    Foods.add(datafoods);

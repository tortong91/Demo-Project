...
public class saveKKFarmController extends Controller {
    ...
    public static Result input() {
        ...
        if (myForm1.hasErrors()) {
            ...
            data2 = Recipe.finder.byId(myForm1.get().getRecip().getRid());
            ...
            if (savefood != null) {
                ...
                if (data2 != null) {
                    int update = data2.getAmountCow() - 1;
                    if (update < 0) {
                        flash("more", "สูตรนี้ได้ใช้ครบจำนวนโคขุนที่สูตรกำหนดแล้ว กรุณาเลือกสูตรใหม่หรือจัดสูตรใหม่ !!");
                    } else {

                        data2.setAmountCow(update);
                        Recipe.edit(data2);
                        Recipe.add(data2);
                        int total = myForm1.get().getKg()*30;
                        data1.setMount(total);
                        int total1 =total*6;
